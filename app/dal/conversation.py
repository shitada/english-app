...
def format_transcript_markdown(export_data: dict[str, Any]) -> str:
    """Format a conversation export into a rich Markdown study document."""
    lines: list[str] = []
    topic = export_data.get("topic", "Unknown")
    difficulty = export_data.get("difficulty", "unknown")
    started = export_data.get("started_at", "")
    ended = export_data.get("ended_at", "")
    summary = export_data.get("summary") or {}

    lines.append(f"# Conversation: {topic}")
    lines.append(f"**Difficulty:** {difficulty} | **Date:** {started[:10] if started else 'N/A'}")
    ...
    if isinstance(summary, dict):
        ...
        if summary.get("tip"):
            ...
            lines.append("")
        if summary.get("key_vocabulary"):
            lines.append("## Key Vocabulary")
            for w in summary["key_vocabulary"]:
                lines.append(f"- {w}")
            lines.append("")
        perf = summary.get("performance")
        if isinstance(perf, dict):
            ...
            if perf.get("avg_words_per_message") is not None:
                lines.append(f"- Avg words/message: {perf['avg_words_per_message']}")
            lines.append("")
    ...
    if messages:
        ...
        for msg in messages:
            ...
            if isinstance(feedback, dict) and role == "user":
                if feedback.get("is_correct"):
                    lines.append("> ✅ Grammar correct")
                else:
                    errors = feedback.get("errors") or []
                    for err in errors:
                        ...
                        lines.append(f'> ❌ "{orig}" → "{corr}" — {expl}')
                    suggestions = feedback.get("suggestions") or []
                    for sug in suggestions:
                        lines.append(f'> 💡 "{sug.get("original", "")}" → "{sug.get("better", "")}" — {sug.get("explanation", "")}')
                    if feedback.get("corrected_text"):
                        lines.append(f"> ✏️ Corrected: {feedback['corrected_text']}")
            lines.append("")
