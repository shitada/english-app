...
import { ClipboardList, Mic, RefreshCw, Square } from 'lucide-react';
import { useSpeechRecognition } from '../hooks/useSpeechRecognition';
import { getInstructionPrompt, evaluateInstruction, type InstructionPromptResponse, type InstructionEvaluateResponse } from '../api';

const MAX_SECONDS = 30;

export default function QuickInstructionCard() {
  const speech = useSpeechRecognition({ continuous: true, interimResults: true });

  const [prompt, setPrompt] = useState<InstructionPromptResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [phase, setPhase] = useState<'idle' | 'speaking' | 'evaluating' | 'done'>('idle');
  const [result, setResult] = useState<InstructionEvaluateResponse | null>(null);
  ...
  const fetchPrompt = useCallback(async () => {
    ...
    try {
      ...
      const res = await getInstructionPrompt(difficulty);
      setPrompt(res);
    } catch {
      // ignore
    } finally {
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      const res = await evaluateInstruction({
        task: prompt.task,
        transcript,
  ...
  return (
    <div className="card" style={{ marginBottom: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
        ...
        <h3 style={{ margin: 0, fontSize: '1rem' }}>Instruction Giver</h3>
      </div>

      {loading ? (
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Loading task…</p>
      ) : !prompt ? (
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>No task available.</p>
      ) : phase === 'idle' ? (
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', margin: '0 0 0.75rem' }}>
            Expected steps: ~{prompt.expected_steps}
          </p>
          <button onClick={handleStart} className="btn btn-primary">
            <Mic size={16} /> Give Instructions ({MAX_SECONDS}s)
          </button>
        </div>
      ) : phase === 'speaking' ? (
        <div>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Evaluating your instructions…</p>
      ) : (
        <div>
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
            {[
              { label: 'Sequencing', score: result?.sequencing_score ?? 0 },
              { label: 'Clarity', score: result?.clarity_score ?? 0 },
              { label: 'Complete', score: result?.completeness_score ?? 0 },
          ...
          {result?.model_instructions && (
            <div style={{
              ...
              <p style={{ fontSize: '0.85rem', color: 'var(--text)', margin: 0, whiteSpace: 'pre-line' }}>
                {result.model_instructions}
              </p>
            </div>
          )}
