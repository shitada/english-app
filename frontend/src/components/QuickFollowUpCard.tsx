...
import { getFollowUpPrompt, evaluateFollowUp, type FollowUpPromptResponse, type FollowUpEvaluateResponse } from '../api';

const MAX_SECONDS = 20;

export default function QuickFollowUpCard() {
  const speech = useSpeechRecognition({ continuous: true, interimResults: true });

  const [prompt, setPrompt] = useState<FollowUpPromptResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [phase, setPhase] = useState<'idle' | 'speaking' | 'evaluating' | 'done'>('idle');
  const [result, setResult] = useState<FollowUpEvaluateResponse | null>(null);
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      const res = await evaluateFollowUp(prompt.statement, transcript, elapsed);
      setResult(res);
      setPhase('done');
    } catch {
      setPhase('idle');
  ...
  return (
    <div className="card" style={{ marginBottom: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
        ...
        <h3 style={{ margin: 0, fontSize: '1rem' }}>Quick Follow-Up</h3>
      </div>

      {loading ? (
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Loading statement…</p>
      ) : !prompt ? (
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>No statement available.</p>
      ) : phase === 'idle' ? (
        <div>
          <p style={{ color: 'var(--text)', fontSize: '0.95rem', margin: '0 0 0.25rem' }}>
            🗣️ &ldquo;{prompt.statement}&rdquo;
          </p>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.8rem', margin: '0 0 0.75rem', fontStyle: 'italic' }}>
            Ask a follow-up question about: {prompt.topic_hint}
          </p>
          <button onClick={handleStart} className="btn btn-primary">
            <Mic size={16} /> Ask a Follow-Up ({MAX_SECONDS}s)
          </button>
        </div>
      ) : phase === 'speaking' ? (
        <div>
          <p style={{ color: 'var(--text)', fontSize: '0.9rem', margin: '0 0 0.5rem' }}>
            🗣️ &ldquo;{prompt.statement}&rdquo;
          </p>
        ...
        <div>
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
            {[
              ...
              { label: 'Depth', score: result?.depth_score ?? 0 },
          ...
          {result?.model_questions && result.model_questions.length > 0 && (
            <div style={{
              ...
              <ul style={{ margin: 0, paddingLeft: '1.25rem' }}>
                {result.model_questions.map((q, i) => (
                  <li key={i} style={{ fontSize: '0.85rem', color: 'var(--text)', marginBottom: '0.15rem' }}>{q}</li>
                ))}
              </ul>
            </div>
          )}
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            ...
            <button onClick={handleNewPrompt} className="btn btn-primary">
              <RefreshCw size={14} /> New Statement
            </button>
          </div>
        </div>
      )}
