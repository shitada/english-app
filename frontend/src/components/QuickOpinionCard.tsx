...
import { getOpinionPrompt, evaluateOpinion, type OpinionPromptResponse, type OpinionEvaluateResponse } from '../api';

const MAX_SECONDS = 30;

export default function QuickOpinionCard() {
  const speech = useSpeechRecognition({ continuous: true, interimResults: true });

  const [prompt, setPrompt] = useState<OpinionPromptResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [phase, setPhase] = useState<'idle' | 'speaking' | 'evaluating' | 'done'>('idle');
  const [result, setResult] = useState<OpinionEvaluateResponse | null>(null);
  ...
  const fetchPrompt = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getOpinionPrompt('intermediate');
      setPrompt(res);
    } catch {
      // ignore
    } finally {
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      const res = await evaluateOpinion(prompt.question, transcript, elapsed);
      setResult(res);
      setPhase('done');
    } catch {
      setPhase('idle');
  ...
  return (
    <div className="card" style={{ marginBottom: '1rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
        ...
        <h3 style={{ margin: 0, fontSize: '1rem' }}>Quick Opinion</h3>
      </div>
      ...
      {loading ? (
        ...
        <div>
          ...
          {prompt.discourse_markers.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem', marginBottom: '0.75rem' }}>
              {prompt.discourse_markers.map((m, i) => (
                <span key={i} style={{
          ...
          <button onClick={handleStart} className="btn btn-primary">
            <Mic size={16} /> Share Opinion ({MAX_SECONDS}s)
          </button>
        </div>
      ) : phase === 'speaking' ? (
        <div>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Evaluating your opinion…</p>
      ) : (
        <div>
